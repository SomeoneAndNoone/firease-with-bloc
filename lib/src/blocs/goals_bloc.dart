...
class GoalsBloc {
  ...
  Stream<String> get goalMessage => _goalMessage.stream.transform(_validateMessage);
  ...
  final _validateMessage =
      StreamTransformer<String, String>.fromHandlers(handleData: (goalMessage, sink) {
  ...
  final _validateName =
      StreamTransformer<String, String>.fromHandlers(handleData: (String name, sink) {
  ...
  void submit(String email) {
    ...
    _repository.uploadGoal(email, _title.value, _goalMessage.value).then((value) {
